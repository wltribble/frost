...
urlpatterns = [
    ...
    url(r'^(?P<jmouniqueid>[0-9]+)/$', PickTemplateView.as_view(), name='pick_template'),
    url(r'^(?P<pk>[0-9]+)/detail/$', DetailView.as_view(), name='detail'),
    url(r'^(?P<jmouniqueid>[0-9]+)/save/$', save_data, name='save_data'),
    url(r'^(?P<jmouniqueid>[0-9]+)/edit/$', edit_data, name='edit_data'),
    url(r'^(?P<jmouniqueid>[0-9]+)/add/$', add_field, name='add_field'),
    url(r'^(?P<jmouniqueid>[0-9]+)/delete/$', delete_field, name='delete_field'),
