            ...
            for operation in center_operations:
                if operation.employee_id == current_operator_employee_id:
                    if operation.operation_id == 0 and operation.assembly_id == 0:
                        ...
                        if operation.end_time == None or operation.start_time < operator.start_time or operation.start_time > operator.start_time + datetime.timedelta(days=1):
                            pass
                        else:
...
class DataView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['centers'] = WorkCenter.objects.all().exclude(workcenter_id=context['current_center'])
        return context
